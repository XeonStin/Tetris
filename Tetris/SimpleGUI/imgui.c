...
void setButtonColors(char *frame, char*label, const char *hotFrame, const char *hotLabel, int fillflag)
{
...
void setMenuColors(char *frame, char*label, const char *hotFrame, const char *hotLabel, int fillflag)
{
...
void setTextBoxColors(char *frame, char*label, const char *hotFrame, const char *hotLabel, int fillflag)
{
...
void usePredefinedColors(int difficultyLevel)
{
	int N = sizeof(gs_predefined_colors)/sizeof(gs_predefined_colors[0]);
	gs_menu_color    = gs_predefined_colors[difficultyLevel%N];
	gs_button_color  = gs_predefined_colors[difficultyLevel%N];
	gs_textbox_color = gs_predefined_colors[difficultyLevel%N];
}
void usePredefinedButtonColors(int difficultyLevel)
{
	int N = sizeof(gs_predefined_colors)/sizeof(gs_predefined_colors[0]);
	gs_button_color  = gs_predefined_colors[difficultyLevel%N];
}
void usePredefinedMenuColors(int difficultyLevel)
{
	int N = sizeof(gs_predefined_colors)/sizeof(gs_predefined_colors[0]);
	gs_menu_color    = gs_predefined_colors[difficultyLevel%N];
}
void usePredefinedTexBoxColors(int difficultyLevel)
{
	int N = sizeof(gs_predefined_colors)/sizeof(gs_predefined_colors[0]);
	gs_textbox_color = gs_predefined_colors[difficultyLevel%N];
}
 ...
 *   1 - 点击了此菜单项 
 */
static int menuItem(int id, double x, double y, double w, double h, const char *label)
{
 ...
 *
 */
int menuList(int id, double x, double y, double w, double wlist, double h, const char *labels[], int n)
{
	...
	if( gs_UIState.keyModifiers & KMOD_CTRL ) {
		for( difficultyLevel=1; difficultyLevel<n; difficultyLevel++ ) {
			int kp = ToUpperLetter( gs_UIState.keyPress );
			if( kp && kp == shortcutkey(labels[difficultyLevel]) ) {
				gs_UIState.keyPress = 0;
				break;
			}
		}
	...
	if( difficultyLevel>0 && difficultyLevel<n ) 
	{	// 成功匹配快捷键
		unfoldMenu = 0;
	...
	if( gs_UIState.actingMenu == id && unfoldMenu  ) {
		...
		for( difficultyLevel=1; difficultyLevel<n; difficultyLevel++ ) {
			if ( menuItem(GenUIID(difficultyLevel)+id, x, y-difficultyLevel*h, wlist, h, labels[difficultyLevel]) ) {
				unfoldMenu = 0;
 ...
 *       }
 *
 *   id  - 唯一号，一般用GenUUID(0), 或用GenUUID（difficultyLevel)（difficultyLevel是循环变量）
 *   x,y - 文本框位置
 *   w,h - 文本框的宽度和高度
...
void drawBox(double x, double y, double w, double h, int fillflag, const char *label, char labelAlignment, const char *labelColor)
{
...
/* 显示字符串标签 */
void drawLabel(double x, double y, const char *label)
{
	if( label && strlen(label)>0 ) {
		MovePen(x,y);
		DrawTextString(label);
